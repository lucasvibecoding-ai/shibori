...
async function grantCourseAccess(
  ...
  try {
    const grantRes = await fetch(`${process.env.COURSE_PLATFORM_URL}/api/grant-access`, {
      ...
      body: JSON.stringify({
        email,
        courseSlug: 'shibori-masterclass',
  ...
  if (event.type === 'payment_intent.succeeded') {
    ...
    after(async () => {
      try {
        ...
        const toEmail = primaryEmail || 'hello@shiboriclass.com';
        ...
        const [access, secondaryAccess, invoice] = await Promise.all([
          grantCourseAccess(primaryEmail),
          secondaryEmail
            ? grantCourseAccess(secondaryEmail, true)
          ...
          createFiscalInvoiceWithin(
            {
              ...
              description: 'Shibori Masterclass',
        ...
        for (const recipient of recipients) {
          ...
          try {
            ...
            const subject = 'Your Shibori Course is ready!';
